...
// This shit pasted from uc, not finished and doesnt work yet

void CAimbotProjectile::GetProjectileBounds(CBaseCombatWeapon* pWeapon, Vector* vMins, Vector* vMaxs)
{
	switch (pWeapon->GetWeaponID())
	{
	case TF_WEAPON_CROSSBOW:
	{
		*vMins = Vector(-3.0f, -3.0f, -3.0f);
		*vMaxs = Vector(3.0f, 3.0f, 3.0f);
		break;
	}
	case TF_WEAPON_COMPOUND_BOW:
	case TF_WEAPON_JAR:
	...
	case TF_WEAPON_SHOTGUN_BUILDING_RESCUE:
	{
		*vMins = Vector(-1.0f, -1.0f, -1.0f);
		*vMaxs = Vector(1.0f, 1.0f, 1.0f);
		break;
	}
	default:
	{
		*vMins = Vector(0.0f, 0.0f, 0.0f);
		*vMaxs = Vector(0.0f, 0.0f, 0.0f);
		break;
	}
	}
}

bool CAimbotProjectile::RaycastProjectileTrajectory(CBaseEntity* pLocal, CBaseEntity* pTarget, const ETargetType targetType, const Vector& vShootPos, const Vector& vProjVel, const float fProjGravity, const float fTime,  const bool bDraw, const float fRes /* 0,1 - 1,0 */) //trace& traceOut,
{
	...
	Vec3 vStartPos = vShootPos;
	Vec3 vStartVel = vProjVel;
	...
	auto iCurWeapon = pWeapon->GetActiveWeapon();
	auto iPrevWeapon = iCurWeapon;
	...
	for (int i = 0; i < NUM_STEPS; i++)
	{
		...
		CGameTrace traceOut;
		Utils::TraceHull(vStepPos, vNextPos, vMins, vMaxs, MASK_SHOT_HULL, &traceFilter, &traceOut);
		vStepPos = traceOut.vEndPos;
		if ( bDraw )
		{
			I::DebugOverlay->AddLineOverlay(vStepPos, vLastStepPos, 96, 96, 96, true, drawTime);
 
			if ( traceOut.DidHit() || bLastStep )
			{
				//if ( traceOut.DidHit() )
					//Math::VectorAngles(traceOut.normal, vaStepAngle); FIX THIS
...
std::optional<Vec3> CAimbotProjectile::GetAimPosBuilding(CBaseEntity* pLocal, CBaseEntity* pEntity)
{
	...
	const float bboxScale = std::max(0.95f - 0.05f, 0.5f); // set the maximum scale for buildings at .95f
